...
- **Add trigger routing to foundry-research skill**: Add TRIGGER/DO NOT TRIGGER directives to skill description to prevent models from bypassing the skill layer and calling MCP research tools directly
- **Rename `deep` workflow to `deep research`**: Update all user-facing references in SKILL.md and CLAUDE.md for clarity
...
- **Migrate note capture to journal tool**: Replaced deprecated `authoring intake-add` with `journal add` across all docs and skills
  - Updated CLAUDE.md autonomous capture instructions
  - Updated troubleshooting docs with journal-based capture syntax
  - Updated foundry-implement SKILL.md MCP tooling table and note capture section
- Removed `skills/foundry-implement/references/note.md` (replaced by journal-based capture)
- Removed `specs/.bikelane/intake.jsonl` (deprecated intake system)
- Updated `.gitignore` to remove bikelane exception
...
- **Deep research polling protocol**: Mandatory polling rules embedded directly in SKILL.md
  - Table of required user communication at each status check (max 5 checks)
  - Explicit "NO INDEPENDENT RESEARCH" rule while deep research is running
  - Anti-pattern examples in deep-research-workflow.md (rapid polling, redundant WebSearch/WebFetch)
  - Good pattern examples showing paced polling with user communication
